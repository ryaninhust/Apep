...
class DNN(object):
    ...
    def epoch_update(self, input_batch, output_batch):
        ...
        origin_weights = [w.copy() for w in self.weights]
        origin_biases = [b.copy() for b in self.biases]

        self.scale_gradient(self.momentum)
        self.update_weights(self.weights, self.biases, origin_weights,
                            origin_biases, self.weight_grads, self.bias_grads)
